...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
     ...
     # Update LED indicators immediately when entering HEAT state
        self.updateLights()
    ...
    def on_exit_heat(self):
     ...
     # Ensure red LED is turned off when leaving HEAT state
     # Prevents residual PWM pulse from continuing in background
        redLight.off()
    ...
    def on_enter_cool(self):
     ...
     # Update LED indicators immediately when entering COOL state
        self.updateLights()
    ...
    def on_exit_cool(self):
     ...
     # Ensure blue LED is turned off when leaving COOL state
     # Prevents residual PWM pulse from continuing in background
        blueLight.off()
    ...
    def on_enter_off(self):
     ...
     # OFF state requires both indicators disabled
    ...
    def processTempStateButton(self):
        ...
        # Trigger state transition: OFF → HEAT → COOL → OFF
        self.cycle()
    ...
    def processTempIncButton(self):
        ...
        # Increase temperature setpoint by 1°F
        self.setPoint += 1

        # Re-evaluate LED behavior immediately after setpoint change
        self.updateLights()
    ...
    def updateLights(self):
        ...
        # OFF state: no indicator lights active
        if self.current_state.id == "off":
            return

        # HEAT state logic:
        # If temp < setPoint  → heater ON  → red LED pulses
        # If temp >= setPoint → heater OFF → red LED solid
        ...
        # COOL state logic:
        # If temp > setPoint  → cooling ON  → blue LED pulses
        # If temp <= setPoint → cooling OFF → blue LED solid
    ...
    def setupSerialOutput(self):
        """
        Formats thermostat status string for UART transmission.
        ...
        # Determine current state as lowercase string
        state_str = "off"
        if self.current_state.id == "heat":
            state_str = "heat"
        ...
        # Capture current temperature in whole degrees Fahrenheit
        temp_f = floor(self.getFahrenheit())

        # Build comma-delimited output string
        output = f"{state_str},{temp_f},{self.setPoint}\n"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            # Line 1 must always show date + time (fit to 16 chars)
            ...
            if(altCounter < 6):
                ...
                # Line 2 (mode A): show current temperature in °F
                # Keep output within 16 characters for LCD width.
                temp_f = self.getFahrenheit()
                lcd_line_2 = f"Temp: {temp_f:4.1f}F"
                lcd_line_2 = lcd_line_2[:16]

                altCounter = altCounter + 1
            else:

                # Line 2 (mode B): show state + setpoint in °F
                state_str = "OFF"
                if self.current_state.id == "heat":
                    state_str = "HEAT"
                elif self.current_state.id == "cool":
                ...
                lcd_line_2 = f"{state_str} SP:{self.setPoint:3d}F"
                lcd_line_2 = lcd_line_2[:16]
            ...
            if((counter % 30) == 0):
                ...
                # Transmit comma-delimited status over UART every 30 seconds
...
## Bind toggle button to the state machine cycle event
...
# Bind increase button to setpoint increment handler
...
# Bind decrease button to setpoint decrement handler
